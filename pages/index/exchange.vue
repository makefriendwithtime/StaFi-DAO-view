<template>
	<scroll-view class="page-view" scroll-y>
		<view class="ex-input" style="margin-top: 5vw;">
			<view style="width: 100%; height: auto; overflow: hidden;">
				...
				<view class="ex-right">
					<text>余额：12.68</text>
					<input type="text" placeholder="输入数量" v-model="num" placeholder-style="color:rgba(255,255,255,.6);">
				</view>
			</view>
		</view>
		<view class="tishi">提示：每次交易将收取10%手续费进入奖池</view>
		<view class="exchang-btn" @click="exBtn">立即交易</view>
...
<script>
	export default {
		...
		methods: {
			...
			toMynft() {
				uni.navigateTo({
					url: '/pages/index/mynft'
				})
			},
			exBtn() {
				if (!this.num) {
					this.$u.toast('请输入交易数量')
				} else {
					this.content = '确认交易' + this.num+ 'ECToken?'
					this.shows = true
				}
			},
			confirm() {
